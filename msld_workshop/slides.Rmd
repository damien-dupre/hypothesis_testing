...
`r faa("exclamation-triangle", animate="flash", speed="slow", color="red")` When using the Omnibus ANOVA Test Table, the estimate cannot be interpreted. Only the p-value is useful.
...
```{r out.width='60%'}
include_graphics("img/jamovi_regression_fit.png")
```

While the main effects of $salary$ and $gender$ as well as their interaction predict 22.7% of the variance of $job\,satisfaction$, the predictions from this model is are not-significantly better than without these effects $(R^2 = 0.227$, $F(3,14) = 1.368$, $p = 0.293)$.
...
```{r out.width='40%'}
include_graphics("img/jamovi_regression_coefficients.png")
```

$job\,satisfaction$ increases by less than 0.001 when $salary$ increases by €1. However, the effect of $salary$ on $job\,satisfaction$ is not statistically significant, therefore $H_0$ cannot be rejected $(b = 0.000, 95\% CI [-0,000, 0.000]$, $t(14) = 0.659$, $p = 0.520)$.

$job\,satisfaction$ of male employees is higher by 2.138 compared to the $job\,satisfaction$ of their female colleagues. However, the effect of $gender$ on $job\,satisfaction$ is not statistically significant, therefore $H_0$ cannot be rejected $(b = 2.138, 95\% CI [-0,241, 4.517]$, $t(14) = 1.927$, $p = 0.074)$.

The effect of $salary$ on $job\,satisfaction$ of male employees is lower by less than 0.001 compared to the effect of $salary$ on $job\,satisfaction$ of females employees. In addition, the interaction effect between $salary$ and $gender$ on $job\,satisfaction$ is not statistically significant, therefore $H_0$ cannot be rejected $(b = -0.000, 95\% CI [-0,000, 0.000]$, $t(14) = -1.701$, $p = 0.111)$.
